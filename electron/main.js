...
// or <app>/resources (win/linux). In an unpackaged dev run (`npm start`),
// there are no extraResources — look for the raw `pyinstaller` output in
// the project's own dist-backend/ instead (`npm run build:backend`).
function getBackendExecutable() {
  ...
  if (!app.isPackaged) {
    return path.join(__dirname, "..", "dist-backend", "pakupaku-backend", execName);
  }
