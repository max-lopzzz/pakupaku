...
def _require_api_key() -> str:
    api_key = (USDA_API_KEY or "").strip()
    if not api_key:
        raise HTTPException(
            status_code=503,
            detail="USDA API key is not configured on the server.",
        )
    return api_key
...
async def get_foods_bulk(
    ...
    if any(fdc_id <= 0 for fdc_id in fdc_ids):
        raise HTTPException(
            status_code=400,
            detail="All fdc_ids must be positive integers.",
        )
